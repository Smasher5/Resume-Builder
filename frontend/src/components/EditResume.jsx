...
import { AlertCircle, ArrowLeft, Check, Download, Loader2, MoveDiagonal, Palette, Save, Trash2 } from 'lucide-react'
...
const EditResume = () => {
  ...
  //Take Back to Previous Form or Take Back to First or to Dashboard
  const goBack = () => {
    const pages = [
      "profile-info",
      "contact-info",
  ...
  //Adding new item in information
  ...
  //Will help in choosing the preview as well as helps in downloading resume also saves resume as an image
  const uploadResumeImages = async () => {
  setIsLoading(true)
  let newThumbnailLink = resumeData.thumbnailLink || "" // Default to existing link

  // 1. Try to generate/upload image
  try {
    const thumbnailElement = thumbnailRef.current
    if (!thumbnailElement) throw new Error("Thumbnail element not found")

    // Helper to fix colors (assuming you imported this util)
    ...
    // Clean up the DOM modification from fixTailwindColors
    ...
    // If successful, update the link variable
    newThumbnailLink = uploadResponse.data.thumbnailLink

  } catch (error) {
    console.error("Image generation/upload failed:", error)
    // We do NOT return here. We continue so we can save the text data.
    toast.error("Thumbnail failed, but saving resume data...")
  }

  // 2. Save the Text Data (Forms) - This runs even if image fails
  try {
    await updateResumeDetails(newThumbnailLink)
  ...
  // This will help in updating resume. put will help in updating on backend
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ position: "absolute", left: "-9999px", top: 0 }} ref={thumbnailRef}>
